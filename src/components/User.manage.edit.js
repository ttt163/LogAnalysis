...
class ManagementEdit extends Component {
    submitUser(e){
        ...
        var backData=sendUserInfo({...jsonData,...userInfo,"action":"edit"});
        if(backData.code==0){
            $("#user_edit_modal").modal("hide");
            new ShowDiag({
                msg: !backData.info?'修改成功！':backData.info,
                closetime: 2
            });
            dispatch(addUserList({"list":getUserList({...jsonData,"uservalue":""})}));
        }else{
            new ShowDiag({
                msg: !backData.info?'修改失败！':backData.info,
                closetime: 2
            });
        }
    }
    render() {
        ...
        return (
            <div className="modal fade" id="user_edit_modal" role="dialog" aria-labelledby="myModalLabel"
                 aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <h4 className="modal-title">编辑用户</h4>
                        </div>
...
export default connect(getData)(ManagementEdit);